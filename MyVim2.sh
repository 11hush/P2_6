...
GetKey_Cmd()
{
    ...
    while [ $fir -eq 1 ] || read -s -N1 key; do 
        if [ $fir -ne 1 ]; then
...
GetKey_Normal()
{
    ...
    j)
        Do_Delete_EOL ;;
	w)
		first_blank_pos=$(echo ${current_line:$c_xpos} | grep -bo "[[:space:]]" | cut -d':' -f1 | head -n 1)
		if [ ${#first_blank_pos} -ne 0 ]; then
			pos=$(echo ${current_line:$((first_blank_pos + c_xpos))} | grep -bo "[[:graph:]]" | cut -d':' -f1 | head -n 1)
			if [ ${#pos} -ne 0 ]; then
				c_xpos=$((c_xpos+pos+first_blank_pos))
			fi
		fi ;;
    *)
        MoveCursor ;;
...
Do_Delete_EOL()
{
    if [ $c_ypos -lt $line_cnt ]; then
        next_line=$(cat $backup | sed -n $((c_ypos+2))'p')
